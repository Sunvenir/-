<template>
  ...
  <div id="editBox1" ref="container">
    <canvas id="canvas1"></canvas>
    <div id = "cutter1" :style="getCutterPos()" @mousedown=drag($event)>
      <canvas id="inner1"></canvas>
    </div>
    <div class="block">
...
export default {
  ...
  methods: {
    ...
    showImage(){
      this.hasImg = false;
      var canvas = document.getElementById("inner1")
      canvas.height = 160;//内部canvas和截取框等大
      canvas.width = 240;
      ...
      img.onload = function(){
        ...
        _that.$emit('renewCover',_that.dataUrl)
        _that.hasImg = true;
        canvas.height = 0;
        canvas.width = 0;
      }
